...
def caracter_invalido(link, titulo):
    return link != titulo and reduce(
        lambda pasa, y: pasa and y not in link,
        ["\n", SEPARADOR_VERTICE, SEPARADOR_ARISTAS],
        True,
    )
...
def parse_page(page, output, n):
    ...
    links = filter(
        lambda x: caracter_invalido(x, title),
        map(lambda link: link[2:-2].split("|")[0], re.findall("\[\[[^\]]*\]\]", page)),
    )
