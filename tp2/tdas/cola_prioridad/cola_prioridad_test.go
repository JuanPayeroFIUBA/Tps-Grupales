...
func TestCrearHeapArrVacio(t *testing.T) {
	t.Log("Testea que en un heap recien creado a partir de un arreglo se comporte correcamente y se puedan realizar operaciones asicas sobre este")
	heap := TDAHeap.CrearHeapArr([]int{}, cmpEnteros)
	require.True(t, heap.EstaVacia())
	require.EqualValues(t, 0, heap.Cantidad())
	require.PanicsWithValue(t, "La cola esta vacia", func() { heap.VerMax() })
	require.PanicsWithValue(t, "La cola esta vacia", func() { heap.Desencolar() })
